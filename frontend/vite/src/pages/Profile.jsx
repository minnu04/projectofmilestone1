import React, { useEffect, useState } from "react";
 ...
 export default function Profile() {
 	...
 	useEffect(() => {
 		fetch(
 			`http://localhost:2204/user/checklogin`,
 			{
 				method: "GET",
 				headers: {
 	...
 	const changeProfilePhoto = async (e) => {
 		...
 		try {
 			const response = await axios.post(
 				"http://localhost:2204/user/upload",
 				multiPartFormData,
 				{
 					headers: {
 					...
 					<div className="w-full h-max my-2 p-5">
 						...
 						<div className="w-full h-max flex flex-col sm:flex-row p-5 gap-10">
 							<div className="w-40 h-max flex flex-col justify-center items-center gap-y-3">
 								...
 								<img
 									src={profilePhoto?`http://localhost:2204/profile-photo/${profilePhoto}`: `https://cdn.vectorstock.com/i/500p/17/61/male-avatar-profile-picture-vector-10211761.jpg`}
 									alt="profile"
